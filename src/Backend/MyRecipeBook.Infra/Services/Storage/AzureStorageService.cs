...
namespace MyRecipeBook.Infra.Services.Storage;
...
public class AzureStorageService : IBlobStorageService
{
    ...
    public async Task<string> GetImageUrl(User user, string fileName)
    {
        ...
        if (exist.Value.IsFalse()) return string.Empty;
        ...
        var sasBuilder = new BlobSasBuilder
        {
            ...
            BlobName = fileName,
            Resource = "b",
            ExpiresOn = DateTimeOffset.UtcNow.AddMinutes(MyRecipeBookRuleConstants.MaximumImageUrlLifetimeInMinutes)
        };
